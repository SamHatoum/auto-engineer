...
import { useThemeConfig } from '@/components/common/active-theme';
...
const THEMES = baseColors.filter((theme) => !['slate', 'stone', 'gray', 'zinc'].includes(theme.name));
  ...
  return (
    <div className={cn('flex w-full items-center gap-2', className)}>
      ...
      <div className="flex items-center gap-2 lg:hidden">
        ...
        <Select value={activeTheme === 'default' ? 'neutral' : activeTheme} onValueChange={setActiveTheme}>
          <SelectTrigger
            id="theme-selector"
            size="sm"
          ...
          <SelectContent align="end">
            <SelectGroup>
              {THEMES.map((theme) => (
                <SelectItem key={theme.name} value={theme.name} className="capitalize data-[state=checked]:opacity-50">
                  {theme.name}
                </SelectItem>
              ))}
            </SelectGroup>
...
function CustomizerCode({ themeName }: { themeName: string }) {
  ...
  const activeTheme = React.useMemo(() => baseColors.find((theme) => theme.name === themeName), [themeName]);
  const activeThemeOKLCH = React.useMemo(() => baseColorsOKLCH[themeName as keyof typeof baseColorsOKLCH], [themeName]);
  ...
  return (
    <>
      <Tabs value={tailwindVersion} onValueChange={setTailwindVersion} className="min-w-0 px-4 pb-4 md:p-0">
        <TabsList>
