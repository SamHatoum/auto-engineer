...
export interface ReactorSpecification<Event, Command, Context extends { commandSender: CommandSender } = {
...
interface Processor<Context> {
    handle: (events: unknown[], context: Context) => Promise<unknown>;
}

interface Reactor<Context> {
    handle: (messages: unknown[], context: Context) => MessageHandlerResult | Promise<MessageHandlerResult>;
}


function reactorSpecificationFor<Event, Command, Context extends { commandSender: CommandSender }>(
    processorOrReactor: Processor<Context> | Reactor<Context>,
...
function assertCommandsMatch<Command>(actual: Command, expected: Command, index?: number): void {
    ...
    if (actualCopy.metadata !== undefined && expectedCopy.metadata !== undefined) {
        for (const key in expectedCopy.metadata) {
            ...
            const expectedStr = expectedValue === undefined
                ? 'undefined'
                : expectedValue === null
                    ? 'null'
