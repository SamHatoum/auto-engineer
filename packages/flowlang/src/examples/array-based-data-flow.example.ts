...
flow('E-commerce Order Flow', () => {
  ...
  commandSlice('Process order')
    .server(() => {
      data([
        // Event sink with field selection and stream routing
        sink().event('OrderProcessed')
          .fields({ orderId: true, customerId: true })
          .toStream('orders-${orderId}'),
        
        // Command sink to external integration
        sink().command('SendOrderEmail')
          .toIntegration(EmailService),
        
        // State sink to database
        sink().state('OrderSummary')
          .toDatabase('order_summaries')
      ]);
    });

  // Query slice with multiple sources
  querySlice('Order dashboard')
    .server(() => {
      data([
        ...
        source().state('CustomerOrders')
          .fromDatabase('orders', { 
            status: { $in: ['pending', 'processing'] } 
          }),
        
        // From external API
        source().state('ShippingStatus')
          .fromApi('https://shipping.api/status', 'GET')
      ]);
    });

  // React slice mixing sources and sinks
  reactSlice('Handle payment confirmation')
    .server(() => {
      data([
        ...
        source().state('CustomerPreferences')
          .fromReadModel('CustomerPreferencesModel'),
        
        // Send payment command
        sink().command('ProcessPayment')
          .toIntegration(PaymentGateway),
