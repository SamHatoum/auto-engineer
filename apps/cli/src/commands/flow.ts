...
export const createFlowCommand = (config: Config, analytics: Analytics) => {
  ...
  return new Command('flow')
    ...
    .action(async () => {
      try {
        output.debug('Flow command initiated');
        ...
        if (selectedFlowIndex === -1) {
          ...
          try {
            ...
            await analytics.trackCommand('flow', true);
            output.debug('Flow command completed successfully with full specs');
            return;
        ...
        await analytics.trackCommand('flow', true);
        output.debug('Flow command completed successfully');

      } catch (error: unknown) {
        await analytics.trackCommand('flow', false, error instanceof Error ? error.message : 'unknown');
